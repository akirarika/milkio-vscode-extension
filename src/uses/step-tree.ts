...
export const useStepTree = (context: vscode.ExtensionContext) => {
    ...
    const disposable = vscode.commands.registerCommand("milkio.steps-to", async (document: vscode.TextDocument, line: number) => {
...
const onStepTreeDocment = async (element?: SchemaItem): Promise<Array<SchemaItem>> => {
    const items: Array<SchemaItem> = []; const editor = vscode.window.activeTextEditor;
    ...
    if (!element) {
        ...
        while (currentline < maxline) {
                ...
                if (element.label === "Action" && content[tmpLine].trim().startsWith("*/")) {
                    while (true) {
                        ...
                        if (content[tmpLine].trim().startsWith("*")) {
                            title = `step: ${content[tmpLine].trim().replace(/^\*+/, '').trim()}`
                            startline = tmpLine;
                        } else break;
                    }
                } else {
            ...
            } else if (element.label === "Tests" && (text.trim().startsWith("handler:") || text.trim().startsWith("handler(") || text.trim().startsWith("async handler("))) {
                let tmpLine = currentline - 1;

...
> => {
	...
	const checkResult = (() => {
		...
		const $io0 = (input: any): boolean =>
			"milkio@logger" === input.type && Array.isArray(input.log);
		const $io1 = (input: any): boolean =>
			"milkio@template" === input.type &&
			"string" === typeof input.name &&
			"string" === typeof input.fsPath &&
			"string" === typeof input.template;
		const $iu0 = (input: any): any =>
			(() => {
				if ("milkio@logger" === input.type) return $io0(input);
				else if ("milkio@template" === input.type) return $io1(input);
				else return false;
			})();
		const $vo0 = (
			input: any,
		...
		const $pu0 = (input: any): any =>
			(() => {
				if ("milkio@logger" === input.type) return $po0(input);
				else if ("milkio@template" === input.type) return $po1(input);
				else
					$throws({
						expected: "(__type | __type.o1)",
						value: input,
		...
		const __is = (input: any): input is CookbookActionParams =>
			"object" === typeof input && null !== input && $iu0(input);
		let errors: any;
		let $report: any;
		const __validate = (
			input: any,
		): typia.IValidation<CookbookActionParams> => {
			if (false === __is(input)) {
				errors = [];
...
> => {
	...
	const checkResult = (() => {
		...
		const __is = (input: any): input is CookbookSubscribeEmits =>
			"object" === typeof input && null !== input && $iu0(input);
		let errors: any;
		let $report: any;
		const __validate = (
			input: any,
		): typia.IValidation<CookbookSubscribeEmits> => {
			if (false === __is(input)) {
				errors = [];
				$report = (typia.misc.validatePrune as any).report(errors);
				((input: any, _path: string, _exceptionable: boolean = true) =>
